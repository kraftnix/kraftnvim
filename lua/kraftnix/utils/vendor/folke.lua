...
-- From folke/dot
function Folke.exists(fname)
  local stat = vim.loop.fs_stat(fname)
  return (stat and stat.type) or false
end

function Folke.fqn(fname)
  fname = vim.fn.fnamemodify(fname, ":p")
  return vim.loop.fs_realpath(fname) or fname
end

function Folke.clipman()
  local file = Folke.fqn("~/.local/share/clipman.json")
  if Folke.exists(file) then
...
function Folke.set_user_var(key, value)
  io.write(string.format("\027]1337;SetUserVar=%s=%s\a", key, Folke.base64(value)))
end
